...
World4::World4(int id, const string& name, const string& music)
    : levelID(id), levelName(name), musicPath(music) {}

void World4::load(SaveData& saveData) {
...
void World4::handleEvent(const sf::Event& event) {
...
void World4::update(float deltaTime) {
    handleInput(deltaTime);
    applyGravity(deltaTime);
    resolveCollisions();
}

void World4::render(sf::RenderWindow& window) {
    window.draw(ground);
...
bool World4::isComplete() const {
    return levelCompleted;
}

void World4::handleInput(float dt) {
...
void World4::applyGravity(float dt) {
    velocityY += gravity * dt;
    player.move(0.f, velocityY * dt);
}

void World4::resolveCollisions() {
